...
export const wisdomEntryTags = sqliteTable('wisdom_entry_tags', {
  entryId: text('entry_id').notNull().references(() => wisdomEntries.id),
  tagId: text('tag_id').notNull().references(() => wisdomTags.id),
});
```
...
export async function createWisdomEntry(params: {
  ...
  for (const tagName of params.tags) {
    ...
    await db.insert(wisdomEntryTags).values({
      entryId: id,
      tagId,
    }).onConflictDoNothing();
  }
...
export async function updateWisdomEntry(params: {
  ...
  if (params.tags !== undefined) {
    ...
    await db.delete(wisdomEntryTags).where(
      eq(wisdomEntryTags.entryId, params.id)
    );
  ...
  await db.update(wisdomEntries)
    .set(updates)
    .where(eq(wisdomEntries.id, params.id));
...
export async function deleteWisdomEntry(id: string): Promise<void> {
  ...
  await db.delete(wisdomEntryTags).where(
    eq(wisdomEntryTags.entryId, id)
  );
  
  // Delete entry
  await db.delete(wisdomEntries).where(
    eq(wisdomEntries.id, id)
  );
}
```
...
export async function queryWisdom(params: WisdomQuery): Promise<WisdomEntry[]> {
  ...
  if (tags && tags.length > 0) {
    ...
    const tagIds = await db.query.wisdomTags.findMany({
      where: (t, { inArray }) => inArray(t.name, tags.map(t => t.toLowerCase())),
    });
    
    if (tagIds.length === 0) return [];
    
    const entryIds = await db.query.wisdomEntryTags.findMany({
      where: (et, { inArray }) => inArray(et.tagId, tagIds.map(t => t.id)),
    });
...
import { tool } from 'ai';
import { z } from 'zod';
import { createWisdomEntry, updateWisdomEntry, deleteWisdomEntry, queryWisdom } from '../memory/wisdom';
...
export async function getTagSuggestions(partial: string): Promise<string[]> {
  return db.query.wisdomTags.findMany({
