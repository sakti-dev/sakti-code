...
async function acquireLock(
  ...
  const result = await db.update(observationalMemory)
    .set({
      lockOwnerId: ownerId,
...
async function commitBufferedObservations(
  ...
  if (!record || 
      record.lockOwnerId !== ownerId || 
      record.lockOperationId !== operationId) {
    // Lock lost or different operation - do not commit!
    return false;
  }
  
  // Safe to commit
  await db.update(observationalMemory)
    .set({
...
async function searchWithRecency(
  db: Database,
  query: string,
  limit: number
) {
...
> 
> When an agent generates a response, that message *does not yet have an ID* because the stream hasn't finished. The agent cannot link the *current* message to a task.
...
3. **Manual Linking (Retroactive):** The `link` tool should only link *past* messages
...
async function claimTask(taskId: string, threadId: string, sessionId: string) {
  ...
  await db.update(tasks)
    .set({ status: 'in_progress', sessionId })
...
> **Why `update_context`?** The Observer updates *episodic* memory (observations), but *semantic* memory (Working Memory: project context, tech stack, preferences) often requires explicit updates. When the user says "actually, let's use Vitest instead", the agent can update Working Memory directly.
