...
User-editable model-selection profiles stored as JSON in `~/.sakti/agent/profiles.json`. A profile maps runtime modes (`default` required; `intake`/`plan`/`build` optional) to a model reference (`provider` + `model` + optional `thinkingLevel`), with a `defaultProfile` selecting the active one and optional hybrid vision fallback. Projects reference a profile via a nullable `profileId` column.
