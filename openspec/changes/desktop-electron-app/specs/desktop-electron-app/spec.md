...
The package SHALL provide `dev` (`electron-vite dev`), `build` (`electron-vite build`), and `package` (`electron-builder`) scripts. `package` SHALL produce a runnable Linux build that hosts the renderer and the embedded server.
...
#### Scenario: package produces a Linux build
- **WHEN** `electron-builder` runs on Linux
- **THEN** it emits an installable artifact that launches the app window with the embedded server responding on its ephemeral port
