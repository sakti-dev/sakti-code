...
describe("top-level show command", () => {
  ...
  const testDir = path.join(tmpdir(), "sakti-show-command-tmp");
  ...
  it("prints hint and non-zero exit when no args and non-interactive", () => {
    ...
    try {
      ...
      let err: any;
      try {
        execSync(`node ${saktiBin} show`, { encoding: "utf-8" });
      } catch (e) {
        err = e;
      }
      ...
      expect(stderr).toContain("sakti show <item>");
      expect(stderr).toContain("sakti change show");
      expect(stderr).toContain("sakti spec show");
    } finally {
  ...
  it("auto-detects change id and supports --json", () => {
    ...
    try {
      process.chdir(testDir);
      const output = execSync(`node ${saktiBin} show demo --json`, { encoding: "utf-8" });
  ...
  it("auto-detects spec id and supports spec-only flags", () => {
    ...
    try {
      process.chdir(testDir);
      const output = execSync(`node ${saktiBin} show auth --json --requirements`, {
        encoding: "utf-8",
      });
      const json = JSON.parse(output);
  ...
  it("handles ambiguity and suggests --type", async () => {
    ...
    try {
      ...
      let err: any;
      try {
        execSync(`node ${saktiBin} show foo`, { encoding: "utf-8" });
      } catch (e) {
        err = e;
      }
      expect(err).toBeDefined();
  ...
  it("prints nearest matches when not found", () => {
    ...
    try {
      ...
      let err: any;
      try {
        execSync(`node ${saktiBin} show unknown-item`, { encoding: "utf-8" });
      } catch (e) {
        err = e;
      }
      expect(err).toBeDefined();
