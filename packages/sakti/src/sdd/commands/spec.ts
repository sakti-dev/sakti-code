...
export function registerSpecCommand(rootProgram: typeof program) {
  ...
  specCommand.hook("preAction", () => {
    console.error(
      'Warning: The "sakti spec ..." commands are deprecated. Prefer verb-first commands (e.g., "sakti show", "sakti validate --specs").',
    );
  });
    ...
    .action((options: { json?: boolean; long?: boolean }) => {
      try {
        ...
        const specs = readdirSync(SPECS_DIR, { withFileTypes: true })
          ...
          .filter(
            (spec): spec is { id: string; title: string; requirementCount: number } =>
              spec !== null,
          )
