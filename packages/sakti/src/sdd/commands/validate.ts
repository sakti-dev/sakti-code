...
export class ValidateCommand {
  async execute(itemName: string | undefined, options: ExecuteOptions = {}): Promise<void> {
    ...
    await this.validateDirectItem(root, itemName, {
      typeOverride,
      strict: !!options.strict,
      json: !!options.json,
    });
  ...
  private printReport(
    ...
    if (json) {
      const out = {
        ...
        summary: {
          totals: { items: 1, passed: report.valid ? 1 : 0, failed: report.valid ? 0 : 1 },
          byType: {
            [type]: { items: 1, passed: report.valid ? 1 : 0, failed: report.valid ? 0 : 1 },
          },
        },
        version: "1.0",
  ...
  private async runBulkValidation(
    ...
    opts: { strict: boolean; json: boolean; concurrency?: string; noInteractive?: boolean },
  ): Promise<void> {
    ...
    if (opts.json) {
      ...
      if (firstFailure) {
        console.log(`Details: sakti validate ${firstFailure.id} --type ${firstFailure.type}`);
      }
    }
