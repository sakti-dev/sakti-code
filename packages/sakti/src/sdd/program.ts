  ...
  if (json?.enabled) {
    console.log(
      JSON.stringify(
        {
          ...json.payload,
          status: [asStatus(error, json.fallbackCode ?? "command_error")],
        },
        null,
        2,
      ),
...
/**
 * Builds the SDD (spec-driven development) commander program.
 *
 * The caller (the binary entry, `src/cli.ts`) is responsible for supplying the
 * version string — read from package.json via the entry's own location — and
 * for driving `program.parse()` from its `runCli` entry function.
 */
export function buildSddProgram(version: string): Command {
  ...
  program
    ...
    .action(
      async (options?: { specs?: boolean; changes?: boolean; sort?: string; json?: boolean }) => {
        try {
          const root = await resolveRootForCommand({
            json: options?.json,
            failurePayload: options?.specs
              ? { specs: [], root: null }
              : { changes: [], root: null },
          });
          if (!root) {
  ...
  const changeCmd = program.command("change").description("Manage Sakti change proposals");

  // Deprecation notice for noun-based commands
  changeCmd.hook("preAction", () => {
    console.error(
      'Warning: The "sakti change ..." commands are deprecated. Prefer verb-first commands (e.g., "sakti list", "sakti validate --changes").',
    );
  });
  ...
  program
    ...
    .option(
      "--concurrency <n>",
      "Max concurrent validations (defaults to env SAKTI_CONCURRENCY or 6)",
    )
    .option("--no-interactive", "Disable interactive prompts")
    .action(
      async (
