...
import { validateChangeName, createChange } from "../change-utils.js";
...
describe("createChange", () => {
  ...
  describe("creates directory", () => {
    it("should create change directory", async () => {
      ...
      const changeDir = path.join(testDir, ".sakti", "changes", "add-auth");
      const stats = await fs.stat(changeDir);
      expect(stats.isDirectory()).toBe(true);
    });

    it("should create .sakti.yaml metadata file with default schema", async () => {
      await createChange(testDir, "add-auth");

      const metaPath = path.join(testDir, ".sakti", "changes", "add-auth", ".sakti.yaml");
      const content = await fs.readFile(metaPath, "utf-8");
      expect(content).toContain("schema: spec-driven");
      expect(content).toMatch(/created: \d{4}-\d{2}-\d{2}/);
    });

    it("should create .sakti.yaml with custom schema", async () => {
  ...
  describe("creates parent directories if needed", () => {
    it("should create sakti/changes/ directories if they do not exist", async () => {
      const newProjectDir = path.join(testDir, "new-project");
      await fs.mkdir(newProjectDir);

      // sakti/changes/ does not exist yet
      await createChange(newProjectDir, "add-auth");

      const changeDir = path.join(newProjectDir, ".sakti", "changes", "add-auth");
      const stats = await fs.stat(changeDir);
      expect(stats.isDirectory()).toBe(true);
    });
  });
