...
describe("FileSystemUtils", () => {
  ...
  describe("canWriteFile", () => {
    ...
    it.skipIf(process.platform === "win32")(
      "should return false for directory without search permission",
      async () => {
  ...
  describe("joinPath", () => {
    it("should join POSIX-style paths", () => {
      const result = FileSystemUtils.joinPath("/tmp/project", ".claude/commands/sakti/proposal.md");
      expect(result).toBe("/tmp/project/.claude/commands/sakti/proposal.md");
    });

    it("should join Linux home directory paths", () => {
      const result = FileSystemUtils.joinPath(
        "/home/dev/workspace/sakti",
        ".cursor/commands/install.md",
      );
      expect(result).toBe("/home/dev/workspace/sakti/.cursor/commands/install.md");
    });

    it("should join Windows drive-letter paths with backslashes", () => {
      const result = FileSystemUtils.joinPath(
        "C:\\Users\\dev\\project",
        ".claude/commands/sakti/proposal.md",
      );
      expect(result).toBe("C:\\Users\\dev\\project\\.claude\\commands\\sakti\\proposal.md");
    });

    it("should join Windows paths that use forward slashes", () => {
      const result = FileSystemUtils.joinPath(
        "D:/workspace/app",
        ".cursor/commands/sakti-apply.md",
      );
      expect(result).toBe("D:\\workspace\\app\\.cursor\\commands\\sakti-apply.md");
    });

    it("should join UNC-style Windows paths", () => {
      const result = FileSystemUtils.joinPath(
        "\\server\\share\\repo",
        ".windsurf/workflows/sakti-archive.md",
      );
      expect(result).toBe("\\server\\share\\repo\\.windsurf\\workflows\\sakti-archive.md");
    });
  });
});
