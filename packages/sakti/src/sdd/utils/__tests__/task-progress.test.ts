...
import os from "os";
import { getTaskProgressForChange } from "../task-progress.js";
import { resolveArtifactOutputs } from "../../core/artifact-graph/index.js";
...
describe("getTaskProgressForChange (#1202 tracked-tasks resolution)", () => {
  ...
  beforeEach(async () => {
    projectRoot = path.join(
      os.tmpdir(),
      `sakti-taskprogress-${Date.now()}-${Math.round(performance.now())}`,
    );
  ...
  it("scopes resolution to the change dir (excludes archive/ and sibling changes)", async () => {
    ...
    await fs.writeFile(
      path.join(changesDir, "archive", "old", "tasks.md"),
      "- [x] x\n- [x] y\n",
      "utf-8",
    );
