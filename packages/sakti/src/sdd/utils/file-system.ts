...
async function hasWritableModeAndAccess(targetPath: string): Promise<boolean> {
  try {
    ...
    // POSIX root can often write despite mode bits, but Sakti should respect
    ...
    if (
      ...
      stats.isDirectory() &&
      !hasOwnerGroupOrOtherExecuteBit(stats)
    ) {
      return false;
    }

    const accessMode = stats.isDirectory() ? fsConstants.W_OK | fsConstants.X_OK : fsConstants.W_OK;
  ...
  static async ensureWritePermissions(dirPath: string): Promise<boolean> {
    try {
      ...
      const testFile = path.join(
        dirPath,
        ".sakti-test-" + Date.now() + "-" + Math.random().toString(36).slice(2),
      );
 ...
 */
export function removeMarkerBlock(content: string, startMarker: string, endMarker: string): string {
