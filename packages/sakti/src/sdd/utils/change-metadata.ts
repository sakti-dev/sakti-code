...
export const METADATA_FILENAME = ".sakti.yaml";
 ...
 */
export function validateSchemaName(schemaName: string, projectRoot?: string): string {
...
/**
 * Writes change metadata to .sakti.yaml in the change directory.
 *
...
/**
 * Reads change metadata from .sakti.yaml in the change directory.
 *
 * @param changeDir - The path to the change directory
 * @param projectRoot - Optional project root for project-local schema resolution
 ...
 */
export function readChangeMetadata(changeDir: string, projectRoot?: string): ChangeMetadata | null {
  const metaPath = path.join(changeDir, METADATA_FILENAME);
 ...
 * 2. Schema from .sakti.yaml metadata (if exists)
 * 3. Schema from sakti/config.yaml (if exists)
...
export function resolveSchemaForChange(
  ...
  // Derive project root from changeDir (changeDir is typically projectRoot/sakti/changes/change-name)
