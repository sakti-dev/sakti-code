...
export class Validator {
        ...
        for (const block of plan.added) {
          ...
          if (!requirementText) {
            ...
            issues.push({
              level: "ERROR",
              path: entryPath,
              message: this.buildMissingShallOrMustMessage(`ADDED "${block.name}"`, block.name),
            });
          }
        ...
        for (const block of plan.modified) {
          ...
          if (!requirementText) {
            ...
            issues.push({
              level: "ERROR",
              path: entryPath,
              message: this.buildMissingShallOrMustMessage(`MODIFIED "${block.name}"`, block.name),
            });
          }
          const scenarioCount = this.countScenarios(block.raw);
          if (scenarioCount < 1) {
   ...
   * present in the spec. Per the Sakti conventions the keyword has to live
