...
- **THEN** qux`;
      ...
      expect(
        report.issues.some(
          (i) =>
            i.level === "ERROR" &&
            i.message.includes('Requirement header "### Requirement: B" appears outside'),
        ),
      ).toBe(true);
    });
...
- **THEN** the quoted delta header remains an example only`;
      ...
      expect(report.valid).toBe(true);
      expect(report.issues.some((i) => i.message.includes("Main spec contains delta header"))).toBe(
        false,
      );
      expect(
        report.issues.some((i) =>
          i.message.includes("appears outside the main ## Requirements section"),
        ),
      ).toBe(false);
    });
  });
...
**Then** it is logged`;
      ...
      expect(report.valid).toBe(false);
      const shallMessage = report.issues.find((i) =>
        i.message.includes("must contain SHALL or MUST"),
      );
