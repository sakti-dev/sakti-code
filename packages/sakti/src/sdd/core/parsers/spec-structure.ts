...
export function findMainSpecStructureIssues(content: string): MainSpecStructureIssue[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (DELTA_HEADER.test(line)) {
      issues.push({
        ...
        message:
          ...
          "Delta headers are only valid inside sakti/changes/<name>/specs/<capability>/spec.md " +
    ...
    const insideRequirements =
      requirementsHeaderIndex !== -1 && i > requirementsHeaderIndex && i < requirementsEndIndex;
...
function isClosingFence(line: string, activeFence: { marker: "`" | "~"; length: number }): boolean {
