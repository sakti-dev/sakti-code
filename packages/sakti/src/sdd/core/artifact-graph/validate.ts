...
import { getProjectSchemasDir, getUserSchemasDir, getPackageSchemasDir } from "./resolver.js";
 ...
 */
export function checkSchemaLocations(name: string, projectRoot: string): SchemaLocation[] {
 ...
 */
export function resolveSchemaLocation(name: string, projectRoot: string): SchemaResolution | null {
...
export function validateSchema(schemaDir: string): SchemaValidationResult {
  ...
  try {
    ...
    const message =
      err instanceof SchemaValidationError ? err.message : `Parse error: ${(err as Error).message}`;
