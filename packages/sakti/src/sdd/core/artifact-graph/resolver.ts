...
/**
 * Cached package root, resolved by walking up from this module's location to
 * find the nearest `package.json` belonging to @sakti-code/sakti. Robust to
 * both bundled output (dist/cli.mjs — flat) and source layout (src/.../resolver.ts — deep).
 */
...
function getPackageRoot(): string {
  ...
  // Fallback: dist entries live one level under the package root.
  cachedPackageRoot = path.dirname(startDir);
...
export function getPackageSchemasDir(): string {
  return path.join(getPackageRoot(), "schemas");
}
...
export function getProjectSchemasDir(projectRoot: string): string {
  return path.join(projectRoot, ".sakti", "schemas");
}
 ...
 * 1. Project-local: <projectRoot>/sakti/schemas/<name>/schema.yaml
 * 2. User override: ${XDG_DATA_HOME}/sakti/schemas/<name>/schema.yaml
