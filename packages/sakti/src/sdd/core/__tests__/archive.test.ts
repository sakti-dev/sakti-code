...
describe("ArchiveCommand", () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = path.join(os.tmpdir(), `sakti-archive-test-${Date.now()}`);
  ...
  describe("execute", () => {
    ...
    it("detects incomplete tasks in nested glob tasks.md files (#1202 data-safety gate)", async () => {
      ...
      await fs.writeFile(path.join(changeDir, ".sakti.yaml"), "schema: glob-tasks\n");
...
### Requirement: Thumbnail Field`;
      ...
      const mainSpecPath = path.join(tempDir, ".sakti", "specs", "gift-card", "spec.md");
      const updatedContent = await fs.readFile(mainSpecPath, "utf-8");
...
Modified content.`;
      ...
      const mainSpecPath = path.join(tempDir, ".sakti", "specs", "new-capability", "spec.md");
...
- TO: \`### Requirement: New Name\``;
      ...
      // Verify spec was NOT created
      const mainSpecPath = path.join(tempDir, ".sakti", "specs", "another-capability", "spec.md");
      await expect(fs.access(mainSpecPath)).rejects.toThrow();
...
Then expected result happens`;
    ...
    it("should support header trim-only normalization for matching", async () => {
      ...
      const mainSpecDir = path.join(tempDir, ".sakti", "specs", "alpha");
...
Updated details.`;
    ...
    it("should apply operations in order: RENAMED → REMOVED → MODIFIED → ADDED", async () => {
      ...
      const mainSpecDir = path.join(tempDir, ".sakti", "specs", "beta");
      await fs.mkdir(mainSpecDir, { recursive: true });
...
content D`;
    ...
    it("should abort with error when MODIFIED/REMOVED reference non-existent requirements", async () => {
      ...
      const mainSpecDir = path.join(tempDir, ".sakti", "specs", "gamma");
      await fs.mkdir(mainSpecDir, { recursive: true });
...
### Requirement: Another Missing`;
    ...
    it("should abort with a structural error when target spec hides requirements outside ## Requirements", async () => {
      ...
      const mainSpecDir = path.join(tempDir, ".sakti", "specs", "delta-target");
      await fs.mkdir(mainSpecDir, { recursive: true });
...
- **THEN** qux changes`;
      ...
      expect(console.log).toHaveBeenCalledWith(
        expect.stringContaining(
          "delta-target: target spec is structurally invalid and cannot be updated until fixed:",
        ),
      );
      expect(console.log).toHaveBeenCalledWith(
        expect.stringContaining(
          'Requirement header "### Requirement: B" appears outside the main ## Requirements section.',
        ),
      );
      expect(console.log).toHaveBeenCalledWith("Aborted. No files were changed.");
    ...
    it("should require MODIFIED to reference the NEW header when a rename exists (error format)", async () => {
      ...
      // Main spec with Old
      const mainSpecDir = path.join(tempDir, ".sakti", "specs", "delta");
      await fs.mkdir(mainSpecDir, { recursive: true });
...
new body`;
    ...
    it("should process multiple specs atomically (any failure aborts all)", async () => {
      ...
      // Existing main specs
      const epsilonMain = path.join(tempDir, ".sakti", "specs", "epsilon", "spec.md");
...
e1`,
      ...
      const zetaMain = path.join(tempDir, ".sakti", "specs", "zeta", "spec.md");
...
### Requirement: Missing`,
    ...
    it("should display aggregated totals across multiple specs", async () => {
      ...
      // Existing main specs
      const omegaMain = path.join(tempDir, ".sakti", "specs", "omega", "spec.md");
      ...
      const psiMain = path.join(tempDir, ".sakti", "specs", "psi", "spec.md");
  ...
  describe("error handling", () => {
    it("should throw error when sakti directory does not exist", async () => {
      ...
      await expect(archiveCommand.execute("any-change", { yes: true })).rejects.toThrow(
        "No Sakti changes directory found. Run 'sakti init' first.",
      );
    });
  });
