...
function gatherConfigFindings(
  rootPath: string
): { present: boolean; valid: boolean; schemaName?: string; findings: DoctorFinding[] } {
...
function gatherSchemaFindings(
  schemaName: string,
  projectRoot: string
): { resolvable: boolean; resolution?: SchemaResolution; validation?: SchemaValidationResult; findings: DoctorFinding[] } {
  const findings: DoctorFinding[] = [];
...
function printHumanReport(report: DoctorReport): void {
  ...
  if (!report.root.found) {
    ...
    console.log('  Fix: Run this command from inside a Sakti project, or run `sakti new change` to initialize one.');
    return;
  }
