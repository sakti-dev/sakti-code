 ...
 * Update Command
 *
 * Refreshes Sakti skills and commands for configured tools.
...
import { AI_TOOLS, SAKTI_DIR_NAME } from './config.js';
import {
  generateCommands,
...
export class UpdateCommand {
  ...
  async execute(projectPath: string): Promise<void> {
    ...
    const saktiPath = path.join(resolvedProjectPath, SAKTI_DIR_NAME);

    // 1. Check sakti directory exists
    if (!await FileSystemUtils.directoryExists(saktiPath)) {
      throw new Error(`No Sakti directory found. Run 'sakti init' first.`);
    }

    // 2. Perform one-time migration if needed before any legacy upgrade generation.
    // Use detected tool directories to preserve existing sakti skills/commands.
    ...
    if (configuredTools.length === 0 && newlyConfiguredTools.length === 0) {
      ...
      console.log(chalk.dim('Run "sakti init" to set up tools.'));
      return;
    }
    ...
    const toolStatuses = configuredTools.map((toolId) => {
      const status = getToolVersionStatus(resolvedProjectPath, toolId, SAKTI_VERSION);
    ...
    if (updatedTools.length > 0) {
      console.log(chalk.green(`✓ Updated: ${updatedTools.join(', ')} (v${SAKTI_VERSION})`));
    }
    ...
    if (newlyConfiguredTools.length > 0) {
      ...
      console.log('  /sakti:new       Start a new change');
      console.log('  /sakti:continue  Create the next artifact');
      console.log('  /sakti:apply     Implement tasks');
      console.log();
  ...
  private displayUpToDateMessage(toolStatuses: ToolVersionStatus[]): void {
    ...
    console.log(chalk.green(`✓ All ${toolStatuses.length} tool(s) up to date (v${SAKTI_VERSION})`));
  ...
  private displayUpdatePlan(
    ...
    const updates = toolsToUpdate.map((toolId) => {
      ...
      if (status?.needsUpdate) {
        ...
        return `${status.toolId} (${fromVersion} → ${SAKTI_VERSION})`;
      }
  ...
  private detectNewTools(projectPath: string, configuredTools: string[]): void {
    ...
    if (newTools.length > 0) {
      ...
      console.log(
        chalk.yellow(
          `Detected new ${toolNoun}: ${newToolNames.join(', ')}. Run 'sakti init' to add ${pronoun}.`
        )
      );
    }
  }
  ...
  private displayExtraWorkflowsNote(
    ...
    if (extraWorkflows.length > 0) {
      console.log(chalk.dim(`Note: ${extraWorkflows.length} extra workflows not in profile (use \`sakti config profile\` to manage)`));
    }
  }
  ...
  private displayOldCoreCustomProfileNote(profile: Profile, workflows?: readonly string[]): void {
    ...
    console.log(chalk.dim('Run `sakti config profile core` and then `sakti update` to add sync.'));
  }
  ...
  /**
   * Detect and handle legacy Sakti artifacts.
