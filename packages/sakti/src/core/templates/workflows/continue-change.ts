...
export function getContinueChangeSkillTemplate(): SkillTemplate {
  return {
    ...
    description: 'Continue working on an Sakti change by creating the next artifact. Use when the user wants to progress their change, create the next artifact, or continue their workflow.',
...
**Input**: Optionally specify a change name after \`/sakti:continue\` (e.g., \`/sakti:continue add-auth\`). If omitted, check if it can be inferred from conversation context. If vague or ambiguous you MUST prompt for available changes.
...
3. **Act based on status**:
   ...
   **If all artifacts are complete (\`isComplete: true\`)**:
   - Congratulate the user
   - Show final status including the schema used
   - Suggest: "All artifacts created! You can now implement this change with \`/sakti:apply\` or archive it with \`/sakti:archive\`."
   - STOP
...
- Current progress (N/M complete)
- What artifacts are now unlocked
- Prompt: "Run \`/sakti:continue\` to create the next artifact"
