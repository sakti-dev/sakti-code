...
describe('store root selection for normal commands', () => {
  ...
  beforeEach(async () => {
    tempDir = fs.realpathSync.native(
      fs.mkdtempSync(path.join(os.tmpdir(), 'sakti-store-root-selection-'))
    );
    env = {
  ...
  function createChange(
    ...
    const changeDir = path.join(rootDir, '.sakti', 'changes', name);
  ...
  describe('selecting a registered store by id', () => {
    it('creates a change only in the store and names the root on stderr', async () => {
      ...
      expect(result.stdout).toContain(
        path.join(storeRoot, '.sakti', 'changes', 'add-billing')
      );

      expect(
        fs.existsSync(path.join(storeRoot, '.sakti', 'changes', 'add-billing'))
      ).toBe(true);
      expectNoLocalSakti();
    });
    ...
    it('lists specs from the store with minimal JSON support', async () => {
      const specDir = path.join(storeRoot, '.sakti', 'specs', 'billing');
      fs.mkdirSync(specDir, { recursive: true });
      fs.writeFileSync(
    ...
    it('archives a change into the store archive with JSON output', async () => {
      ...
      expect(json.archive.path).toBe(
        path.join(storeRoot, '.sakti', 'changes', 'archive', json.archive.archivedAs)
      );
      ...
      expect(
        fs.existsSync(path.join(storeRoot, '.sakti', 'changes', 'store-change'))
      ).toBe(false);
      expect(
        fs.existsSync(path.join(storeRoot, '.sakti', 'specs', 'billing', 'spec.md'))
      ).toBe(true);
      expectNoLocalSakti();
    });
  });
  ...
  describe('human output and stdout purity', () => {
    ...
    it('writes the status banner to stderr in human mode', async () => {
      ...
      expect(result.stdout).not.toContain('Using Sakti root');
    });
  });
  ...
  describe('selector errors', () => {
    it('rejects --store-path with register guidance', async () => {
      ...
      expectNoLocalSakti();
      expect(fs.existsSync(path.join(storeRoot, '.sakti', 'changes', 'nope'))).toBe(false);
    });
  ...
  describe('default resolution without --store', () => {
    it('fails with a store hint instead of scaffolding when no root exists', async () => {
      ...
      expect(output).toContain('--store <id>');
      expect(output).toContain('sakti init');
      expectNoLocalSakti();
    });

    it('treats leftover workspace state as no root at all', async () => {
      fs.mkdirSync(path.join(appRepo, '.sakti-workspace'), { recursive: true });
      fs.writeFileSync(
        path.join(appRepo, '.sakti-workspace', 'view.yaml'),
        'version: 1\nname: platform\ncontext: null\nlinks: {}\n'
      );
    ...
    it('ignores leftover workspace state when a nearby root exists', async () => {
      ...
      createSaktiRoot(localRepo);
      fs.mkdirSync(path.join(localRepo, '.sakti-workspace'), { recursive: true });
      fs.writeFileSync(
        path.join(localRepo, '.sakti-workspace', 'view.yaml'),
        'version: 1\nname: platform\ncontext: null\nlinks: {}\n'
      );
      createChange(localRepo, 'local-change');
  ...
  describe('archive --json is non-interactive', () => {
    ...
    it('reports validation failures as diagnostics without stdout prose', async () => {
      ...
      expect(
        fs.existsSync(path.join(storeRoot, '.sakti', 'changes', 'bad-change'))
      ).toBe(true);
    });
    ...
    it('writes no spec when any rebuilt spec fails validation', async () => {
      ...
      const badTargetDir = path.join(storeRoot, '.sakti', 'specs', 'zzz-bad');
      ...
      expect(
        fs.existsSync(path.join(storeRoot, '.sakti', 'specs', 'aaa-good', 'spec.md'))
      ).toBe(false);
      expect(fs.readFileSync(path.join(badTargetDir, 'spec.md'), 'utf-8')).toBe(
        badTargetContent
      );
      expect(
        fs.existsSync(path.join(storeRoot, '.sakti', 'changes', 'two-spec-change'))
      ).toBe(true);
    });
    ...
    it('reports spec-update failures as diagnostics without stdout prose', async () => {
      ...
      expect(
        fs.existsSync(path.join(storeRoot, '.sakti', 'changes', 'modified-change'))
      ).toBe(true);
    });
    ...
    it('refuses incomplete tasks without --yes', async () => {
      ...
      expect(
        fs.existsSync(path.join(storeRoot, '.sakti', 'changes', 'wip-change'))
      ).toBe(true);
    });
  });
  ...
  describe('initiative links are retired from normal change flows', () => {
    it('rejects --initiative and creates no files', async () => {
      ...
      expect(
        fs.existsSync(path.join(localRepo, '.sakti', 'changes', 'linked-change'))
      ).toBe(false);
    });

    it('removes sakti set change entirely', async () => {
      ...
      expect(help.stdout).not.toContain('Set checked-in Sakti metadata');
  ...
  describe('setup and register point to --store usage', () => {
    it('shows --store usage after setup', async () => {
      ...
      expect(result.exitCode).toBe(0);
      expect(result.stdout).toContain('sakti new change <change-id> --store fresh-context');
    });

    it('shows --store usage after register', async () => {
      const registerRoot = path.join(tempDir, 'register-context');
      createSaktiRoot(registerRoot);
      ...
      expect(result.exitCode).toBe(0);
      expect(result.stdout).toContain('sakti new change <change-id> --store register-context');
    });
  });
});
