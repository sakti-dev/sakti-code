...
describe('PowerShellGenerator', () => {
	...
	describe('generate', () => {
		it('should generate valid PowerShell completion script with header', () => {
			...
			expect(script).toContain('# PowerShell completion script for Sakti CLI');
			expect(script).toContain('$saktiCompleter = {');
			expect(script).toContain('Register-ArgumentCompleter');
		});

		it('should register argument completer for sakti command', () => {
			const commands: CommandDefinition[] = [
				{
					name: 'init',
					description: 'Initialize Sakti',
			...
			expect(script).toContain('Register-ArgumentCompleter -CommandName sakti');
			expect(script).toContain('-ScriptBlock $saktiCompleter');
		});
		...
		it('should handle positional arguments for change-id', () => {
			...
			expect(script).toContain('Get-SaktiChanges');
		});
		...
		it('should handle positional arguments for spec-id', () => {
			...
			expect(script).toContain('Get-SaktiSpecs');
		});
		...
		it('should handle positional arguments for schema names', () => {
			...
			expect(script).toContain('Get-SaktiSchemas');
			expect(script).toContain('sakti __complete schemas 2>$null');
		});
		...
		it('should generate dynamic completion helper for changes', () => {
			...
			expect(script).toContain('function Get-SaktiChanges');
			expect(script).toContain('sakti __complete changes 2>$null');
		...
		it('should generate dynamic completion helper for specs', () => {
			...
			expect(script).toContain('function Get-SaktiSpecs');
			expect(script).toContain('sakti __complete specs 2>$null');
		});
