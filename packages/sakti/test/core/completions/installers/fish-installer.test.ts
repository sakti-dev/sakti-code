...
describe('FishInstaller', () => {
  ...
  beforeEach(async () => {
    testHomeDir = path.join(os.tmpdir(), `sakti-fish-test-${randomUUID()}`);
  ...
  describe('getInstallationPath', () => {
    it('should return standard fish completions path', () => {
      ...
      expect(result).toBe(path.join(testHomeDir, '.config', 'fish', 'completions', 'sakti.fish'));
    });
    ...
    it('should use homeDir from constructor', () => {
      ...
      expect(result).toBe(path.join(customHome, '.config', 'fish', 'completions', 'sakti.fish'));
    });
  });
  ...
  describe('install', () => {
    const mockCompletionScript = `# Fish completion script for Sakti CLI
function __fish_sakti
    echo "test"
end

complete -c sakti -a 'init' -d 'Initialize Sakti'
`;
    ...
    it('should install completion script for the first time', async () => {
      ...
      expect(result.installedPath).toBe(path.join(testHomeDir, '.config', 'fish', 'completions', 'sakti.fish'));
    ...
    it('should create parent directories if they do not exist', async () => {
      ...
      expect(result.success).toBe(true);
      const targetPath = path.join(testHomeDir, '.config', 'fish', 'completions', 'sakti.fish');
    ...
    it('should update when content is different', async () => {
      ...
      const updatedScript = `# Fish completion script for Sakti CLI
function __fish_sakti_new
    echo "updated"
end

complete -c sakti -a 'init' -d 'Initialize Sakti'
complete -c sakti -a 'validate' -d 'Validate specs'
`;
    ...
    it('should handle installation with paths containing spaces', async () => {
      const spacedHomeDir = path.join(os.tmpdir(), `sakti fish test ${randomUUID()}`);
      ...
      expect(result.success).toBe(true);
      expect(result.installedPath).toContain('sakti fish test');
