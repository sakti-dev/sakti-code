...
describe('migration', () => {
  ...
  beforeEach(async () => {
    projectDir = path.join(os.tmpdir(), `sakti-migration-project-${randomUUID()}`);
    configHome = path.join(os.tmpdir(), `sakti-migration-config-${randomUUID()}`);
  ...
  it('migrates to custom skills delivery when only managed skills are detected', async () => {
    await writeSkill(projectDir, 'sakti-explore');
    await writeSkill(projectDir, 'sakti-apply-change');
  ...
  it('ignores unknown custom skill and command files when scanning workflows', async () => {
    ...
    const customCommandPath = path.join(projectDir, '.claude', 'commands', 'sakti', 'my-custom.md');
