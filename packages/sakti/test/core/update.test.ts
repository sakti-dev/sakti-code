...
import { SAKTI_MARKERS } from '../../src/core/config.js';
  ...
  describe('basic validation', () => {
    it('should throw error if sakti directory does not exist', async () => {
      ...
      await expect(updateCommand.execute(testDir)).rejects.toThrow(
        "No Sakti directory found. Run 'sakti init' first."
      );
    });
...
`;
      ...
      expect(updatedSkill).toContain('name: sakti-explore');
    ...
    it('should update core profile skill files when tool is configured', async () => {
      ...
      await fs.writeFile(
        path.join(skillsDir, 'sakti-explore', 'SKILL.md'),
        'old content'
      );
  ...
  describe('multi-tool support', () => {
    it('should update multiple configured tools', async () => {
      ...
      await fs.mkdir(path.join(claudeSkillsDir, 'sakti-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(claudeSkillsDir, 'sakti-explore', 'SKILL.md'),
        'old'
      );

      // Set up Cursor
      const cursorSkillsDir = path.join(testDir, '.cursor', 'skills');
      await fs.mkdir(path.join(cursorSkillsDir, 'sakti-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(cursorSkillsDir, 'sakti-explore', 'SKILL.md'),
        'old'
      );
      ...
      expect(claudeSkill).toContain('name: sakti-explore');
      ...
      expect(cursorSkill).toContain('name: sakti-explore');
    ...
    it('should update Qwen tool with correct command format', async () => {
      ...
      await fs.mkdir(path.join(qwenSkillsDir, 'sakti-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(qwenSkillsDir, 'sakti-explore', 'SKILL.md'),
        'old'
      );

      await updateCommand.execute(testDir);

      // Check Qwen command format (TOML) - Qwen uses flat path structure: sakti-<id>.toml
    ...
    it('should update Windsurf tool with correct command format', async () => {
      ...
      await fs.mkdir(path.join(windsurfSkillsDir, 'sakti-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(windsurfSkillsDir, 'sakti-explore', 'SKILL.md'),
        'old'
      );
  ...
  describe('skill content validation', () => {
    ...
    it('should include proper instructions in skill files', async () => {
      ...
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'sakti-apply-change'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(skillsDir, 'sakti-apply-change', 'SKILL.md'),
        'old'
      );
...
`
      ...
      const cursorSkillDir = path.join(testDir, '.cursor', 'skills', 'sakti-explore');
...
`
  ...
  describe('version tracking', () => {
    ...
    it('should only update tools that need updating', async () => {
      ...
      const claudeSkillFile = path.join(testDir, '.claude', 'skills', 'sakti-explore', 'SKILL.md');
  ...
  describe('legacy cleanup', () => {
    it('should detect and auto-cleanup legacy files with --force flag', async () => {
      ...
      // Create legacy CLAUDE.md with Sakti markers
      const legacyContent = `${SAKTI_MARKERS.start}
# Sakti Instructions
...
`;
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Upgrading to the new Sakti')
      );

      // Should show marker removal message (config files are never deleted, only have markers removed)
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Removed Sakti markers from CLAUDE.md')
      );
      ...
      expect(content).not.toContain(SAKTI_MARKERS.end);
...
`;
    ...
    it('should not show legacy cleanup messages when no legacy files exist', async () => {
      ...
      const hasLegacyMessage = calls.some(call =>
        call.includes('Upgrading to the new Sakti')
      );
    ...
    it('should remove Sakti marker block from mixed content files', async () => {
      // Set up a configured tool
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'sakti-explore'), {
        recursive: true,
      ...
      // Create CLAUDE.md with mixed content (user content + Sakti markers)
...
`;
      ...
      expect(updatedContent).not.toContain(SAKTI_MARKERS.start);
      expect(updatedContent).not.toContain(SAKTI_MARKERS.end);
  ...
  describe('legacy tool upgrade', () => {
    it('should upgrade legacy tools to new skills with --force', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('/sakti:new')
      );
    ...
    it('should upgrade multiple legacy tools with --force', async () => {
      ...
      await fs.writeFile(
        path.join(testDir, '.cursor', 'commands', 'sakti-proposal.md'),
        'content'
      );
      ...
      const claudeSkillFile = path.join(testDir, '.claude', 'skills', 'sakti-explore', 'SKILL.md');
      const cursorSkillFile = path.join(testDir, '.cursor', 'skills', 'sakti-explore', 'SKILL.md');
    ...
    it('should not upgrade legacy tools already configured', async () => {
      ...
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'sakti-explore'), { recursive: true });
      await fs.writeFile(
        path.join(skillsDir, 'sakti-explore', 'SKILL.md'),
        'existing skill'
      );
    ...
    it('should upgrade only unconfigured legacy tools when mixed', async () => {
      ...
      const claudeSkillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(claudeSkillsDir, 'sakti-explore'), { recursive: true });
      await fs.writeFile(
        path.join(claudeSkillsDir, 'sakti-explore', 'SKILL.md'),
        'existing skill'
      );
    ...
    it('should create only effective profile skills when upgrading legacy tools', async () => {
      ...
      const nonCoreSkill = path.join(skillsDir, 'sakti-new-change', 'SKILL.md');
  ...
  describe('profile-aware updates', () => {
    it('should generate only profile workflows when custom profile is set', async () => {
      ...
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'sakti-explore'), { recursive: true });
      await fs.writeFile(path.join(skillsDir, 'sakti-explore', 'SKILL.md'), 'old');
      ...
      expect(await FileSystemUtils.fileExists(
        path.join(skillsDir, 'sakti-new-change', 'SKILL.md')
      )).toBe(true);
    ...
    it('should suggest core preset when custom profile preserves the old core workflow set', async () => {
      ...
      expect(calls.some(call =>
        call.includes('sakti config profile core') && call.includes('sakti update')
      )).toBe(true);
...
`
  ...
  describe('new tool detection', () => {
    it('should detect new tool directories not currently configured', async () => {
      ...
      const claudeSkillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(claudeSkillsDir, 'sakti-explore'), { recursive: true });
      await fs.writeFile(path.join(claudeSkillsDir, 'sakti-explore', 'SKILL.md'), 'old');
      ...
      const hasNewToolMessage = calls.some(call =>
        call.includes("Detected new tool: Cursor. Run 'sakti init' to add it.")
      );
    ...
    it('should consolidate multiple new tools into one message', async () => {
      ...
      expect(consolidatedCalls[0]).toContain("Run 'sakti init' to add them.");
  ...
  describe('scanInstalledWorkflows', () => {
    it('should detect installed workflows across tools', async () => {
      ...
      const claudeSkillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(claudeSkillsDir, 'sakti-explore'), { recursive: true });
      await fs.writeFile(path.join(claudeSkillsDir, 'sakti-explore', 'SKILL.md'), 'content');
      await fs.mkdir(path.join(claudeSkillsDir, 'sakti-apply-change'), { recursive: true });
      await fs.writeFile(path.join(claudeSkillsDir, 'sakti-apply-change', 'SKILL.md'), 'content');
    ...
    it('should return union of workflows across multiple tools', async () => {
      ...
      // Cursor has apply
      const cursorSkillsDir = path.join(testDir, '.cursor', 'skills');
      await fs.mkdir(path.join(cursorSkillsDir, 'sakti-apply-change'), { recursive: true });
      await fs.writeFile(path.join(cursorSkillsDir, 'sakti-apply-change', 'SKILL.md'), 'content');
