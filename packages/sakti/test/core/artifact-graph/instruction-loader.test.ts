...
describe('instruction-loader', () => {
  ...
  describe('loadChangeContext', () => {
    ...
    beforeEach(() => {
      tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sakti-test-'));
    });
    ...
    it('should detect completed artifacts', () => {
      ...
      const changeDir = path.join(tempDir, '.sakti', 'changes', 'my-change');
    ...
    it('should auto-detect schema from .sakti.yaml metadata', () => {
...
`
      ...
      it('should deduplicate validation warnings within session', () => {
        ...
        const freshTempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sakti-test-'));
