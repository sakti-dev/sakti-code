...
function main() {
  ...
  try {
    log(`Packing @sakti-code/sakti@${expected}...`);
    const filename = npmPack();
    tgzPath = path.resolve(filename);
    log(`Created: ${tgzPath}`);

    work = mkdtempSync(path.join(tmpdir(), 'sakti-pack-check-'));
    log(`Temp dir: ${work}`);
    ...
    const binRel = path.join('node_modules', '@fission-ai', 'sakti', 'bin', 'sakti.js');
