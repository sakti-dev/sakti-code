...
// Small 2x2 blue JPEG image (base64) - from pi's test fixtures
const TINY_JPEG =
...
describe("resizeImage", () => {
  it("keeps caller input bytes intact", async () => {
  ...
  it("returns original image when within limits", async () => {
  ...
  it("resizes image exceeding byte limit", async () => {
...
describe("formatDimensionNote", () => {
  it("returns undefined for non-resized images", () => {
  ...
  it("returns formatted note for resized images", () => {
    const note = formatDimensionNote({
      data: "",
      mimeType: "image/png",
      originalWidth: 2000,
