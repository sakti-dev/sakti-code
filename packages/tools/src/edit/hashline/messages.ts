...
export function unseenLinesMessage(
  ...
  return `This edit anchors to lines ${ranges} of ${sectionPath} that ${HL_FILE_PREFIX}${sectionPath}${HL_FILE_HASH_SEP}${tag}${HL_FILE_SUFFIX} never displayed (it showed a partial range, a search hit, or a folded summary). Re-read them in full first with a ranged read like \`${sectionPath}:${selector}\` — it skips summarization and mints a fresh tag (a plain re-read just re-folds them) — then re-issue the edit.`;
}
