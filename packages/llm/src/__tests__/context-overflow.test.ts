...
describe("isContextOverflow", () => {
  it("Case 1: error whose message matches an overflow pattern", () => {
    ...
    expect(
      isContextOverflow(asst("error", { errorMessage: "maximum context length is 8192 tokens" })),
    ).toBe(true);
  });
  ...
  it("Case 1: rate-limit / throttling errors are NOT overflow", () => {
    ...
    expect(isContextOverflow(asst("error", { errorMessage: "too many requests" }))).toBe(false);
    ...
    expect(
      isContextOverflow(asst("error", { errorMessage: "Throttling error: Too many tokens" })),
    ).toBe(false);
  });

  it("Case 1: generic 'too many tokens' (no throttling prefix) IS overflow", () => {
    expect(isContextOverflow(asst("error", { errorMessage: "Too many tokens, please wait" }))).toBe(
      true,
    );
  });

  it("Case 1: an error with an unrelated message is not overflow", () => {
    expect(isContextOverflow(asst("error", { errorMessage: "internal server error" }))).toBe(false);
  });
  ...
  it("Case 2: cacheRead counts toward the input total", () => {
    expect(
      isContextOverflow(asst("stop", { usage: { ...usage(500), cacheRead: 600 } }), 1000),
    ).toBe(true);
  });
  ...
  it("Case 3: MiMo length-stop — length + output 0 + input fills window", () => {
    ...
    expect(isContextOverflow(asst("length", { usage: usage(990, 50) }), 1000)).toBe(false);
  });
