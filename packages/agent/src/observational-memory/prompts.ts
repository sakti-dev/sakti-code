/**
 * Observational Memory prompt bundles.
 *
 * Verbatim port from Mastra's observer-agent.ts, reflector-agent.ts, and
 * constants.ts. Adaptation surface is minimal:
 *   - `buildObserverHistoryMessage` consumes `AgentMessage[]` instead of
 *     `MastraDBMessage[]`
 *   - Extractor / multi-thread branches are dropped (single-thread, no
 *     extractors in v1) with params defaulted
...
function safeSlice(str: string, end: number): string {
  ...
  // Try to break at a grapheme boundary (surrogate pair safe)
  const sliced = str.slice(0, end);
  // If the last char is a high surrogate, drop it to avoid lone surrogate
 ...
 * Exported so the Reflector can understand how observations were created.
 */
 ...
 * Exported so the Reflector can reference them.
 */
...
/**
 * Build the output format instructions for the Observer.
 * Without extractors (v1 default), includes current-task and suggested-response.
 */
 ...
 * Single-thread only (no extractors, no multi-thread in v1).
 */
...
/**
 * Build the task prompt for the Observer.
 */
...
// ─── Observer message formatting ─────────────────────────────────────────────

function formatObserverDate(timestamp?: number): string {
  if (!timestamp) return "";
...
function formatObserverTime(timestamp?: number): string {
  if (!timestamp) return "";
  const date = new Date(timestamp);
  if (Number.isNaN(date.getTime())) return "";
...
function formatAgentMessageForObserver(msg: AgentMessage): string {
  const role = msg.role.charAt(0).toUpperCase() + msg.role.slice(1);
  const timestamp = "timestamp" in msg ? (msg.timestamp as number) : undefined;
  const date = formatObserverDate(timestamp);
  const time = formatObserverTime(timestamp);
  ...
  switch (msg.role) {
    case "user": {
      ...
      if (typeof msg.content === "string") {
        lines.push(`${timePrefix}${role}: ${msg.content}`);
      } else if (Array.isArray(msg.content)) {
        const textParts = msg.content.filter(
          (p): p is { type: "text"; text: string } => p.type === "text",
        );
        const imageParts = msg.content.filter(
          (p): p is { type: "image"; data: string; mimeType: string } => p.type === "image",
        );
        const textContent = textParts.map((p) => p.text).join("\n");
        if (textContent) lines.push(`${timePrefix}${role}: ${textContent}`);
        for (let i = 0; i < imageParts.length; i++) {
          lines.push(`[${role} Image #${i + 1}: ${imageParts[i]!.mimeType}]`);
        }
      }
      break;
    }
    ...
    case "assistant": {
      ...
      for (const block of msg.content) {
        if (block.type === "text") {
          lines.push(`${timePrefix}${role}: ${block.text}`);
        } else if (block.type === "toolCall") {
          lines.push(
            `${timePrefix}${role} called ${block.name}: ${JSON.stringify(block.arguments)}`,
          );
        }
        // Skip thinking blocks — not relevant for observation
      }
      break;
    }
    ...
    case "toolResult": {
      ...
      const textContent = msg.content
        ...
        .join("\n");
      const truncated =
        textContent.length > 2000 ? textContent.slice(0, 2000) + "\n... [truncated]" : textContent;
      lines.push(`${timePrefix}${role} (${msg.toolName}): ${truncated || "(empty)"}`);
      break;
    }

    case "bashExecution": {
      const timePrefix = time ? `(${time}) ` : "";
      lines.push(`${timePrefix}${role}: $ ${msg.command}`);
      if (msg.output) {
        const truncated =
          msg.output.length > 1000 ? msg.output.slice(0, 1000) + "\n... [truncated]" : msg.output;
    ...
    case "custom": {
      ...
      lines.push(`${timePrefix}${role} (${msg.customType}): ${content}`);
      break;
    }
...
export function buildReflectorPrompt(
  ...
  const level: CompressionLevel =
    typeof compressionLevel === "number" ? compressionLevel : compressionLevel ? 1 : 0;
 ...
 */
export interface ReflectorParseResult {
  observations: string;
...
/**
 * Parse the Reflector's output to extract observations and suggested response.
 * Uses XML tag parsing for structured extraction.
 */
export function parseReflectorOutput(output: string): ReflectorParseResult {
  if (detectDegenerateRepetition(output)) {
    return { observations: "", degenerate: true };
  }

  const result: { observations: string; suggestedResponse: string } = {
    observations: "",
    suggestedResponse: "",
  };
  ...
  if (observationsMatches.length > 0) {
    ...
    result.observations = listLines.join("\n").trim();
  }
  ...
  return {
    ...
    ...(result.suggestedResponse !== undefined
      ? { suggestedContinuation: result.suggestedResponse }
      : {}),
  };
}

// ─── Injection format constants ──────────────────────────────────────────────
