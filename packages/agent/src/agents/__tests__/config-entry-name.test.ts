...
describe("configEntryNameFromPath", () => {
  ...
  it("strips agent/ and agents/ prefixes", () => {
    expect(configEntryNameFromPath("agent/triage.md", ["agent/", "agents/"])).toBe("triage");
    expect(configEntryNameFromPath("agents/team/triage.md", ["agent/", "agents/"])).toBe(
      "team/triage",
    );
  });

  it("falls back to basename when no prefix matches", () => {
    expect(configEntryNameFromPath("triage.md", ["command/", "commands/"])).toBe("triage");
  });
