...
import type { AgentEvent, AgentMessage, SessionStore, AgentTool } from "../types";
...
function createMockStore(): SessionStore {
  ...
  return {
    ...
    replaceMessages: vi.fn(async (id: string, msgs: AgentMessage[]) => { messages.set(id, [...msgs]); }),
  };
}
...
function toolCallStream(toolName: string, args: Record<string, unknown>, id = "tc_1") {
  const s = new MockEventStream();
  const now = Date.now();
  s.push({ type: "start", partial: { ...basePartial, stopReason: "toolUse", timestamp: now } });
  s.push({ type: "toolcall_start", contentIndex: 0, partial: {} as any });
  s.push({ type: "toolcall_delta", contentIndex: 0, delta: JSON.stringify(args), partial: {} as any });
  s.push({ type: "toolcall_end", contentIndex: 0, toolCall: { type: "toolCall", id, name: toolName, arguments: args }, partial: {} as any });
  s.push({ type: "done", reason: "toolUse", message: {
    ...basePartial, stopReason: "toolUse", timestamp: now,
...
describe("Agent loop event ordering", () => {
  it("emits events in correct order for a simple text turn", async () => {
    ...
    expect(types.indexOf("agent_start")).toBeLessThan(types.indexOf("turn_start"));
    expect(types.indexOf("turn_start")).toBeLessThan(types.indexOf("message_start"));
    expect(types.indexOf("message_start")).toBeLessThan(types.indexOf("message_end"));
    expect(types.indexOf("message_end")).toBeLessThan(types.indexOf("turn_end"));
...
describe("Agent loop tool execution", () => {
  it("executes tool and emits tool_execution events in order", async () => {
    ...
    vi.mocked(streamSimple).mockImplementation(() => {
      callCount++;
      if (callCount === 1) return toolCallStream("echo", { msg: "hello" });
      return textStream("All done!");
    });

    const loop = createAgentLoop({ sessionId: "s1", model: testModel, tools: [echoTool], store });
    ...
    const execEnd = events.find((e): e is Extract<AgentEvent, { type: "tool_execution_end" }> => e.type === "tool_execution_end");
  ...
  it("tool with terminate=true stops the loop", async () => {
    ...
    const loop = createAgentLoop({ sessionId: "s1", model: testModel, tools: [terminateTool], store });
  ...
  it("persists messages to store (user, assistant, tool result)", async () => {
    ...
    vi.mocked(streamSimple).mockImplementation(() => {
      callCount++;
      if (callCount === 1) return toolCallStream("read", { path: "x" });
      return textStream("Done reading");
    });

    const loop = createAgentLoop({ sessionId: "s1", model: testModel, tools: [readTool], store });
