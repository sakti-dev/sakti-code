...
describe("AgentEvent discriminated union", () => {
  it("agent_start event", () => {
    const e: AgentEvent = { type: "agent_start", sessionId: "s1", timestamp: 0 };
  ...
  it("text_delta message update", () => {
    const e: AgentEvent = { type: "message_update", update: { type: "text_delta", delta: "hello" }, timestamp: 0 };
  ...
  it("thinking_delta message update", () => {
    const e: AgentEvent = { type: "message_update", update: { type: "thinking_delta", delta: "hmm" }, timestamp: 0 };
  ...
  it("toolcall message updates", () => {
    const start: AgentEvent = { type: "message_update", update: { type: "toolcall_start", contentIndex: 0 }, timestamp: 0 };
    const delta: AgentEvent = { type: "message_update", update: { type: "toolcall_delta", contentIndex: 0, delta: '{"path"' }, timestamp: 0 };
    const end: AgentEvent = { type: "message_update", update: { type: "toolcall_end", contentIndex: 0, toolCall: { type: "toolCall", id: "tc_1", name: "read", arguments: { path: "x" } } }, timestamp: 0 };
    expect(start.type).toBe("message_update");
    if (delta.type === "message_update" && delta.update.type === "toolcall_delta") expect(delta.update.delta).toBe("{\"path\"");
    if (end.type === "message_update" && end.update.type === "toolcall_end") expect(end.update.toolCall.name).toBe("read");
  });
  ...
  it("error event", () => {
    const e: AgentEvent = { type: "error", message: "LLM failed", timestamp: 0 };
    expect(e.type).toBe("error");
    if (e.type === "error") expect(e.message).toBe("LLM failed");
  });

  it("compaction events", () => {
    const start: AgentEvent = { type: "compaction_start", timestamp: 0 };
    const end: AgentEvent = { type: "compaction_end", tokensBefore: 100000, tokensAfter: 30000, timestamp: 0 };
    expect(start.type).toBe("compaction_start");
    if (end.type === "compaction_end") expect(end.tokensAfter).toBe(30000);
  });

  it("retry event", () => {
    const e: AgentEvent = { type: "retry", attempt: 2, maxRetries: 3, delayMs: 2000, timestamp: 0 };
    if (e.type === "retry") {
  ...
  it("isAgentEvent runtime guard covers all types", async () => {
    ...
    const events: AgentEvent[] = [
      ...
      { type: "message_update", update: { type: "text_delta", delta: "x" }, timestamp: 0 },
      { type: "tool_execution_start", toolCallId: "tc", toolName: "r", timestamp: 0 },
      { type: "tool_execution_update", toolCallId: "tc", accumulated: "p", timestamp: 0 },
      { type: "tool_execution_end", toolCallId: "tc", result: { content: "r", terminate: false }, timestamp: 0 },
      { type: "error", message: "err", timestamp: 0 },
      { type: "compaction_start", timestamp: 0 },
      { type: "compaction_end", tokensBefore: 100, tokensAfter: 50, timestamp: 0 },
      { type: "retry", attempt: 1, maxRetries: 3, delayMs: 1000, timestamp: 0 },
    ];
    for (const e of events) expect(isAgentEvent(e)).toBe(true);
