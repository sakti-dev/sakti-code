...
import { buildHarnessStreamRequest } from "../../harness/build-stream-request.ts";
...
/** Minimal agent-loop request shape (subset of StreamRequest the loop builds). */
...
describe("buildHarnessStreamRequest", () => {
  ...
  it("does not drop maxOutputTokens even when set to a falsy-but-valid value", () => {
    // maxOutputTokens: 0 is nonsensical but the loop could theoretically set it;
    // the conditional spread must still forward when the loop explicitly sets it.
    // We treat 0/undefined as "not set" (matches stream() semantics) — document it.
